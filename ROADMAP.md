...
Последнее обновление: **2026-08-26** (slice 39 — Player Handout Export). Ветка: `main`.

Слайс **2026-09-05**: [Atlas — размеры, группы и слои](docs/ATLAS-EDITING-STATUS.md) — ручки/числовые размеры, групповое выделение и общая отмена, до 12 слоёв с порядком, lock/hide и переносом объектов. Старые карты совместимы; PNG содержит только видимые слои, master JSON — все. Локально проверено: 110 тестов, 9 новых + 8 картографических + 12 campaign browser scenarios, typecheck/lint/build/bundle budget. Подготовлен единый пакет в `main` вместе с предыдущими двумя локальными слайсами ниже; результат CI/production проверяется отдельно. Один прежний Low advisory остаётся открыт.

Локальный follow-up **2026-09-05**: [Map Workspace, A05–A06](docs/MAP-WORKSPACE-STATUS.md) — полноразмерный холст, повторное редактирование сохранённых карт без смены ID/меток, непрерывные комнаты/коридоры и рабочая толщина стен/проёмы. 100 тестов, 8 новых и 12 прежних браузерных сценариев; typecheck/lint/build/bundle budget пройдены. **Не опубликовано**; расширенная геометрия и прежние release/security gates остаются отдельными задачами.

Локальная доработка **2026-09-05**: надёжность кампаний по аудиту A01–A04 — сохранение/восстановление мира, изоляция ресурсов, безопасное архивирование и локальная отмена Atlas. Добавлены библиотека кампаний, master backup/restore и защита от записи из второй вкладки. Проверено: 93 теста, 12 браузерных сценариев, typecheck/lint/build/bundle budget. **Не опубликовано**; один прежний Low dependency advisory и остальные ограничения остаются открыты. Подробности и следующий шаг: [Campaign reliability status](docs/CAMPAIGN-RELIABILITY-STATUS.md). Исторический marker slice 39 сохранён.
...
### v0.3 slice 39 — Player Handout Export

*Реализовано 2026-08-26 как локальный privacy-preserving PNG export.*

- [x] В режиме «Игроки» карта со статусом `allowed` экспортируется в PNG без network request.
- [x] Export plan сначала фильтрует метки по текущему `mapId` и `visibility=table`, затем
  переносит только координаты, тип и название.
- [x] GM-метки, заметки, timestamps и внутренние идентификаторы не входят в handout plan
  и не достигают canvas renderer.
- [x] Квадратная сетка, символы и короткие подписи рисуются локально поверх bounded preview;
  object URL отзывается после скачивания.
- [x] `review-required` и `blocked` закрывают экспорт с явным объяснением причины.

**Текущая граница:** скачанный PNG — самостоятельный файл пользователя, не защищённая
player session. Отзыв доступа, ACL, realtime sync и fog of war требуют серверного слоя.

### v0.3 slice 38 — Map Story Pins

*Реализовано 2026-08-26 как narrative overlay поверх локальных preview.*

- [x] На сохранённую карту можно поставить до 32 нормализованных меток: сцена, улика,
  опасность, находка или переход.
- [x] Каждая метка хранит bounded plain-text название и заметку; управляющие и bidi-символы
  очищаются, неизвестные поля и невалидные координаты отклоняются fail-closed.
- [x] Новая метка по умолчанию видна только мастеру; локальный режим «Игроки» показывает
  только явно открытые table-safe метки.
- [x] Мышиный клик ставит метку в нужную точку, а для клавиатуры есть очевидное действие
  «Метка в центре» с последующим редактированием.
- [x] Карта в состоянии `blocked` не допускает добавления или редактирования производных
  меток; удаление карты каскадно удаляет её локальные Story Pins.

**Текущая граница:** режим «Игроки» — только локальный preview, не ACL и не отдельная
защищённая игровая сессия. Совместная работа, fog of war и network player view остаются
отдельными production slices.

### v0.3 slice 37 — Location Map Workshop MVP

*Реализовано 2026-08-26 как local-first расширение существующего map workflow.*

- [x] Панель карт разделена на понятные режимы «Карта локации» и «Карта мира»; Azgaar import
  сохранён без изменения поведения.
- [x] К локации можно привязать локальный PNG/JPEG/WebP preview, тип сетки, масштаб и размеры;
  исходное изображение не отправляется на сервер.
- [x] Импорт ограничен 8 МБ, preview — 384 КБ; расширение, MIME и magic bytes проверяются,
  SVG/HTML/remote URL не принимаются.
- [x] Versioned `eclipse.location-map-library.v1` хранит до 8 карт и fail-closed отклоняет
  неизвестные поля, повреждённые preview и несогласованное состояние rights gate.
- [x] Provenance фиксирует rights basis, источник/провайдера, лицензию, attribution,
  commercial intent, consent и derivative/IP risk.
- [x] Dungeon Scrawl и другие VTT обозначены как external research references only:
  без копирования UI, кода, assets, `.ds` parsing или заявлений об affiliation.

**Текущая граница:** `allowed` разрешает внутреннюю работу, но не закрывает отдельный
product-trademark gate. Cloud persistence, ACL, полноценный редактор геометрии,
защищённая player session, fog of war и VTT export остаются следующими слайсами.

### v0.3 slice 36 — preliminary trademark screen

*Опубликовано 2026-08-20 как предварительный поиск, не юридическое заключение.*

- [x] Название `Eclipse DnD Forge` имеет статус **NOT CLEARED**; использование DnD/D&D
  сохраняет высокий trademark/affiliation risk.
- [x] Официальные WIPO, EUIPO, USPTO и Роспатент/ФИПС источники дали ограниченный,
  технически неполный screen и не подтверждают доступность знака.
- [x] Billing и commercial marketing остаются закрыты до clearance нового имени
  квалифицированным trademark/patent attorney.
- [x] Evidence и ограничения: [TRADEMARK-PRELIMINARY-SCREEN-2026-08-20.md](docs/TRADEMARK-PRELIMINARY-SCREEN-2026-08-20.md).
...
- **Living Atlas — оригинальный clean-room редактор карт** (Dungeon Scrawl используется только как capability reference, не как шаблон интерфейса, кода, текстов, форматов или assets):
  - базовый контур за одну минуту: pan/zoom, комнаты, коридоры, стены, двери, лестницы, прямые и кривые, square/hex grid;
  - выделение, перемещение, resize, слои, lock/hide, undo/redo, локальное versioned save/load, PNG/WebP export;
  - собственные source-cleared штампы, текст и стили, импорт разрешённых генераторов, процедурный dungeon draft и изометрический режим;
  - player tabletop, fog of war, lighting/line of sight, print/PDF, связанные этажи и большие составные карты;
  - отличие Eclivarium: привязка объектов к NPC/фракциям/квестам/сценам, Story Pins, временные версии мира, последствия сессий и подтверждаемые игроком AI-агенты;
  - обязательные gates: provenance каждого внешнего asset, безопасный импорт, отсутствие копирования `.ds` без отдельного разрешения/открытой спецификации и отдельный commercial-use review.
